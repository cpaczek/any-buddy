...
export function findSalt(userId, desired, { onProgress } = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const workers = Array.from({ length: workerCount }, () => ({
      attempts: 0, elapsed: 0, errors: '', done: false, exit: null,
    }));
    let totalAttempts = 0;
    let maxElapsed = 0;
    let doneCount = 0;
    ...
    function markDone(idx, exit) {
      if (workers[idx].done) return false;
      workers[idx].done = true;
      workers[idx].exit = exit;
      doneCount++;
      return !settled;
    }
    ...
    for (let i = 0; i < workerCount; i++) {
      ...
      child.stderr.on('data', (chunk) => {
        ...
        for (const line of lines) {
          try {
            ...
            totalAttempts += progress.attempts - workers[i].attempts;
            workers[i].attempts = progress.attempts;
            if (progress.elapsed > maxElapsed) maxElapsed = progress.elapsed;
            workers[i].elapsed = progress.elapsed;
            ...
            workers[i].errors += line + '\n';
          }
        }
      });

      child.on('close', (code, sig) => {
        if (!markDone(i, { code, sig })) return;

        if (code === 0 && stdout.trim()) {
          try {
            const result = JSON.parse(stdout.trim());
            totalAttempts += result.attempts - workers[i].attempts;
            workers[i].attempts = result.attempts;
            if (result.elapsed > maxElapsed) maxElapsed = result.elapsed;
            settled = true;
            cleanup();
            resolve({ salt: result.salt, attempts: totalAttempts, elapsed: maxElapsed });
            return;
          } catch {
        ...
        if (doneCount === workerCount) {
          settled = true;
          cleanup();
          const root = workers.map(w => w.exit).find(e => e && typeof e.code === 'number' && e.code !== 0 && !e.sig)
            ?? workers.map(w => w.exit).find(e => e) ?? { code, sig };
          const reason = root.sig ? `killed by ${root.sig}` : `exited with code ${root.code}`;
          const stderrClean = workers
            .map((w, idx) => w.errors.trim() ? `[worker ${idx}] ${w.errors.trim()}` : '')
            .filter(Boolean).join('\n');
          const extra = {
            Bun: bunBinary,
            Workers: workerCount,
      ...
      child.on('error', (err) => {
        if (!markDone(i, { code: null, sig: null })) return;
