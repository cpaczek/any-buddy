...
export async function runInteractive(flags = {}) {
  ...
  let species, eye, rarity, hat, shiny, peak = null, dump = null;

  // --preset flag: look up by name (case-insensitive)
  if (flags.preset) {
    const match = PRESETS.find(p => p.name.toLowerCase() === flags.preset.toLowerCase());
    if (!match) {
      const names = PRESETS.map(p => `"${p.name}"`).join(', ');
      throw new Error(`Unknown preset "${flags.preset}". Available: ${names}`);
    }
  ...
  // If CLI flags specify any attribute, skip preset selection and go straight to manual
  const hasFlags = !species && (flags.species || flags.eye || flags.rarity || flags.hat || flags.shiny || flags.peak || flags.dump);

  if (!species && !hasFlags) {
    const mode = await select({
      message: 'How do you want to pick?',
      choices: [
        { name: 'Browse presets — curated themed builds', value: 'preset' },
        { name: 'Customize — pick each attribute yourself', value: 'custom' },
      ],
    });
  ...
  // Manual selection (or fill in remaining attributes from flags)
  if (!species) {
    species = validateFlag('species', flags.species, SPECIES) ?? await selectSpecies();
    eye = validateFlag('eye', flags.eye, EYES) ?? await selectEyes(species);
    rarity = validateFlag('rarity', flags.rarity, RARITIES) ?? await selectRarity();
    hat = rarity === 'common' ? 'none'
      : validateFlag('hat', flags.hat, HATS) ?? await selectHat(species, eye, rarity);
  }

  shiny = flags.shiny ?? await confirm({
...
async function selectPreset() {
  const chosen = await select({
    ...
    choices: PRESETS.map(p => {
      ...
      const bones = { species: p.species, eye: p.eye, hat: p.hat, rarity: p.rarity, shiny: false };
      const sprite = renderSprite(bones, 0).map(l => '    ' + color(l)).join('\n');
